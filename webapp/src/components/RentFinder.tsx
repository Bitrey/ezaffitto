...
import { cityCoords, config, gaEvents, rentalTypeOptions } from "../config";
...
const RentFinder: FC<RentFinderProps> = ({ city }) => {
  ...
  }, [executeRecaptcha, setIsLoading, t]);
  ...
  useEffect(() => {
    ...
    console.log("Fetching posts by useEffect fetchData()", {
      city,
      limit,
      skip: cursor,
      rentalTypes,
  ...
  const citiesOptions: DropdownOption<EzaffittoCity>[] = ezaffittoCities.map(
    e => ({
      key: e,
      value: t("city." + e)
    })
  ...
  return (
    <div className="pb-8 dark:bg-gray-800 dark:text-white">
      <div className="flex justify-center items-center relative">
        <LazyLoadImage
          key={city}
          src={`/img/${city}.webp`}
          className="animate-slideIn w-full h-[20vh] md:h-[30vh] lg:h-[50vh] object-cover"
        />

        <div className="flex items-center justify-center gap-3 px-5 py-3 text-2xl md:text-3xl lg:text-4xl absolute left-0 right-0 mx-auto bg-[rgba(255,255,255,0.75)] dark:bg-[rgba(25,25,25,0.69)] z-50 text-red-600 text-center font-bold tracking-tighter">
          ...
          {city && (
            <Dropdown
              columns
              enabledOptions={config.enabledCities.map(e => ({
                key: e,
                value: t("city." + e)
              }))}
              onSelectCustom={e => {
